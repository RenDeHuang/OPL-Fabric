...
- The Tencent capacity provider resolves ComputePools by provider instance type, reuses an existing TKE NodePool when found, and can create/verify/delete TKE NodePools through the Tencent Cloud Go SDK when `OPL_TKE_ALLOW_NODEPOOL_MUTATION=true`. The worker resolves the ComputePool/NodePool first, writes `node_pool_id` onto the ComputeAllocation, then lets the runtime create compute, attach storage, and create the stable Workspace URL.
...
- Live staging e2e has proven storage retention and compute rebuild behavior on the earlier path. The current ComputePool resolver path has local and fake-client coverage; it still needs a fresh live staging e2e run before being treated as cloud-proven.
